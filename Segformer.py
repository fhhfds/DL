...
Encoder(编码):
 ...
 - Efficient Self-Attention 轻量的自注意力机制
 - Mix FFNs
Decoder（解码） Head
"""
        ...
        for stage_i in range(len(depths)):
            ...
            for N in range(depths[stage_i]):
                blocks.append(transformer_block(dim=embed_dims[stage_i],
                                                ...
                                                dropout_p=dropout_p,
                                                drop_path_p=drop_path_p * (sum(depths[:stage_i]) + N) / (
                                                        sum(depths) - 1),  # stochastic depth decay rule
...
class segformer_head(nn.Module):
    ...
    def forward(self, x):
        ...
        ups = [unify_stages[0]] + \
              [F.interpolate(stage, size=feature_size, mode='bilinear', align_corners=True) for stage in unify_stages[1:]]
...
## 合并
class segformer_mit_b0(nn.Module):
    def __init__(self, in_channels, num_classes):
        super().__init__()
        # Encoder block    
        self.backbone = mix_transformer(in_chans=in_channels, embed_dims=(32, 64, 160, 256),
                                        num_heads=(1, 2, 5, 8), depths=(2, 2, 2, 2),
                                        sr_ratios=(8, 4, 2, 1), dropout_p=0.1, drop_path_p=0.2)
        # decoder block
        self.decoder_head = segformer_head(in_channels=(32, 64, 160, 256),
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, size=image_hw, mode='bilinear', align_corners=True)
        return x
